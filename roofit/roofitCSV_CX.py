...
##########
  ...
  ttjjEff=0.1165 
###########
  #831.76/0.1049580955*0.020*0.1165
...
#  print ttbbCX
  ...
  k      =RooRealVar(       "k","normalization factor",           ttbarCX, ttbarCX*0.5, ttbarCX*1.5) 
  #k      =RooRealVar(       "k","normalization factor",           ttjjCX*1., ttjjCX*0.5, ttjjCX*1.5) 
  #ttjjcx      =RooRealVar(       "ttjjCX","ttjjCX",           ttjjCX, ttjjCX*0.5, ttjjCX*1.5) 
  #ttbbcx      =RooRealVar(       "ttbbCX","ttbbCX",           ttbbCX, ttbbCX*0.5, ttbbCX*1.5) 
  ...
  cR10 = TCanvas("R10", "R", 1200,400)#500, 500)
  cR10.Divide(3,1)
  cR10.cd(1)
  ...
  RFrame.GetXaxis().SetTitle("R_{reco} as ttbb/ttjj")
  RFrame.SetTitle("")
  RFrame.Draw()
  ...
  genR      = recoR * (ttjjEff/ttbbEff)
  genRerror = recoR*(ttjjEff/ttbbEff)*recoRerror/recoR
  fullR     = (genR*ttjjAcc/ttbbAcc)
  fullRerr  = (genR*ttjjAcc/ttbbAcc)*(genRerror/genR)

  #pt52 = addLegend2("Eff.^{t#bar{t}bb} = "+str(roudV(ttbbEff*100.))+" %" ,0.6,0.70)
  #pt53 = addLegend2("Acc.^{t#bar{t}bb} = "+str(roudV(ttbbAcc*100.))+" %" ,0.6,0.63)
  pt54 = addLegend2("R_{VIS} = "+str(roudV(genR))+" #pm "+str(roudV( genRerror)),0.50,0.55)
  pt55 = addLegend2("R_{FUL} = "+str(roudV(fullR))+" #pm "+str(roudV(fullRerr)),0.50,0.48)
  #pt52.Draw()
  #pt53.Draw()
  pt54.Draw()
  pt55.Draw()
...
########################
  ...
  #ttjjCX_RV = RooRealVar(    "ttjjCX_RV","ttjjCX_RV", ttjjCX, ttjjCX,ttjjCX) 
  #ttjjCX_RF = RooFormulaVar("ttjjCX_RF", "ttjjCX_RF","@0*@1",RooArgList(ttjjCX_RV,k) )
  #modelEff = RooEffProd("modelEff","model with efficiency",model2,ttjjCX_RF) 
  #RFrameK = ttjjCX_RF.frame()
  #nllK.plotOn(RFrameK,RooFit.ShiftToZero())


  RFrameK.SetMaximum(4.);RFrameK.SetMinimum(0)
  RFrameK.GetXaxis().SetTitle("#sigma^{t#bar{t}}")
  RFrameK.SetTitle("")
  ...
  lineTbbK = TLine(ttbarCX,RFrameK.GetMaximum(),ttbarCX,0)
  ...
  l1K.AddEntry(lineTbbK,"prefit: #sigma^{t#bar{t}} = "+str(roudV(ttbarCX)),"l")
  l1K.AddEntry(lineKKK,"fit: #sigma^{t#bar{t}} = "+str(roudV(kVal))+" #pm "+str(roudV(kValerror))+"","l")
  l1K.SetTextSize(0.04)
  l1K.SetFillColor(0)
  l1K.SetLineColor(0)
  l1K.Draw()
 
  pt42 = addLegend2("Eff.^{t#bar{t}jj} = "+str(roudV(ttjjEff*100.))+" %" ,0.6,0.70)
  pt43 = addLegend2("Acc.^{t#bar{t}jj} = "+str(roudV(ttjjAcc*100.))+" %" ,0.6,0.63)
  pt44 = addLegend2("#sigma^{t#bar{t}jj}_{VIS} = "+str(roudV(kVal*ttjjCXVS))+" #pm "+str(roudV(kVal*ttjjCXVS*kValerror/kVal)),0.60,0.55)
  pt45 = addLegend2("#sigma^{t#bar{t}jj}_{FUL} = "+str(roudV(kVal*ttjjCXFS))+" #pm "+str(roudV(kVal*ttjjCXFS*kValerror/kVal)),0.60,0.48)
  pt42.Draw()
  pt43.Draw()
  pt44.Draw()
  pt45.Draw()
  ...
  NewNttbb = n_ttjj * kVal*recoR
  NewNttjj = n_ttjj * kVal

  NewCXttbbvis = NewNttbb/(lumi * ttbbEff )
  NewCXttjjvis = NewNttjj/(lumi * ttjjEff )
  NewCXttbbfull = NewNttbb/(lumi * ttbbEff * ttbbAcc )
  NewCXttjjfull = NewNttjj/(lumi * ttjjEff * ttjjAcc )

 ######ddddddddddd
  pt62 = addLegend2("Eff.^{t#bar{t}b#bar{b}} = "+str(roudV(ttbbEff*100.))+" %" ,0.2,0.90)
  pt63 = addLegend2("Acc.^{t#bar{t}b#bar{b}} = "+str(roudV(ttbbAcc*100.))+" %" ,0.2,0.83)
  pt64 = addLegend2("#sigma^{t#bar{t}bb}_{VIS} = "+str(roudV(kVal*genR*ttjjCXVS ))+" #pm "+str(roudV(sqrt(kValerror/kVal*kValerror/kVal + genRerror/genR*genRerror/genR)*(kVal*genR*ttjjCXVS)  ) ),0.2,0.75)
  pt65 = addLegend2("#sigma^{t#bar{t}bb}_{FUL} = "+str(roudV(kVal*fullR*ttjjCXFS))+" #pm "+str(roudV(sqrt(kValerror/kVal*kValerror/kVal + fullRerr/fullR*fullRerr/fullR)*(kVal*fullR*ttjjCXFS)  ) ),0.2,0.68)

  pt72 = addLegend2("Eff.^{t#bar{t}jj} = "+str(roudV(ttjjEff*100.))+" %" ,0.2,0.50)
  pt73 = addLegend2("Acc.^{t#bar{t}jj} = "+str(roudV(ttjjAcc*100.))+" %" ,0.2,0.43)
  pt74 = addLegend2("#sigma^{t#bar{t}jj}_{VIS} = "+str(roudV(kVal*ttjjCXVS))+" #pm "+str(roudV(kVal*ttjjCXVS*kValerror/kVal)),0.2,0.35)
  pt75 = addLegend2("#sigma^{t#bar{t}jj}_{FUL} = "+str(roudV(kVal*ttjjCXFS))+" #pm "+str(roudV(kVal*ttjjCXFS*kValerror/kVal)),0.2,0.28)
  pt72.Draw()
  pt73.Draw()
  pt74.Draw()
  pt75.Draw()
  ...
  cR10.cd(3).Update()
  cR10.Print("plots/"+GEN+"_sigma_constraintTTB.eps")
  cR10.Print("plots/"+GEN+"_sigma_constraintTTB.png")
  
  """  
  if freeTTB and freeTTCC:
    cR00.Print("plots/"+GEN+"_sigma_freeTTBTTCC.eps")
    cR00.Print("plots/"+GEN+"_sigma_freeTTBTTCC.png") 
  elif freeTTB:
    cR00.Print("plots/"+GEN+"_sigma_freeTTB.eps")
    cR00.Print("plots/"+GEN+"_sigma_freeTTB.png")
  elif freeTTCC:              
    cR00.Print("plots/"+GEN+"_sigma_freeTTCC.eps")
    cR00.Print("plots/"+GEN+"_sigma_freeTTCC.png")
  else :                      
    cR00.Print("plots/"+GEN+"_sigma_constraintTTB.eps")
    cR00.Print("plots/"+GEN+"_sigma_constraintTTB.png")
  """
  """ 
  ################
  ################
